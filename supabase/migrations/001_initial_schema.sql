...
CREATE INDEX idx_convenios_normalized ON convenios(normalized_name);

CREATE TABLE convenio_aliases (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_convenio_aliases_normalized ON convenio_aliases(normalized_alias);
...
CREATE INDEX idx_procedures_tuss ON procedures(tuss_code);
...
CREATE INDEX idx_procedure_aliases_normalized ON procedure_aliases(normalized_alias);
...
CREATE INDEX idx_repasse_rules_lookup ON repasse_rules(clinic_id, convenio_id, category_id);
...
CREATE TABLE conference_sessions (
  ...
  production_upload_id UUID REFERENCES uploads(id),
  repasse_upload_id UUID REFERENCES uploads(id),
...
CREATE INDEX idx_production_date ON production_records(service_date);
CREATE INDEX idx_production_patient ON production_records(patient_name_normalized);
CREATE INDEX idx_production_matching ON production_records(patient_name_normalized, service_date, convenio_id);
...
CREATE INDEX idx_repasse_date ON repasse_records(service_date);
CREATE INDEX idx_repasse_patient ON repasse_records(patient_name_normalized);
CREATE INDEX idx_repasse_matching ON repasse_records(patient_name_normalized, service_date, convenio_id);
...
CREATE TABLE matches (
  ...
  repasse_record_id UUID NOT NULL REFERENCES repasse_records(id),
...
CREATE TABLE divergences (
  ...
  repasse_record_id UUID REFERENCES repasse_records(id),
...
CREATE INDEX idx_divergences_session ON divergences(session_id);
...
INSERT INTO procedure_categories (slug, display_name, generates_repasse, default_repasse_pct) VALUES
...
INSERT INTO procedures (tuss_code, description, category_id) VALUES
  ('10101012', 'Consulta em consultório (no horário normal ou preestabelecido)', (SELECT id FROM procedure_categories WHERE slug = 'consulta')),
  ('10101039', 'Consulta em pronto socorro', (SELECT id FROM procedure_categories WHERE slug = 'consulta')),
  ('20104065', 'Cerúmen - remoção (bilateral)', (SELECT id FROM procedure_categories WHERE slug = 'cerumen')),
  ('30402042', 'Corpos estranhos, pólipos ou biópsia - em consultório', (SELECT id FROM procedure_categories WHERE slug = 'corpo_estranho')),
  ('30205271', 'Adenoidectomia por videoendoscopia', (SELECT id FROM procedure_categories WHERE slug = 'cirurgia')),
  ('30205050', 'Amigdalectomia das palatinas', (SELECT id FROM procedure_categories WHERE slug = 'cirurgia')),
  ('30205069', 'Amigdalectomia lingual', (SELECT id FROM procedure_categories WHERE slug = 'cirurgia')),
  ('30501458', 'Turbinectomia ou turbinoplastia - unilateral', (SELECT id FROM procedure_categories WHERE slug = 'cirurgia')),
  ('30201063', 'Frenectomia labial', (SELECT id FROM procedure_categories WHERE slug = 'cirurgia')),
  ('30200015', 'Frenectomia lingual', (SELECT id FROM procedure_categories WHERE slug = 'cirurgia')),
  ('40201210', 'Vídeo-endoscopia naso-sinusal com ótica flexível', (SELECT id FROM procedure_categories WHERE slug = 'video_naso')),
  ('40201260', 'Vídeo-faringo-laringoscopia com endoscópio rígido', (SELECT id FROM procedure_categories WHERE slug = 'video_laringo')),
  ('40201252', 'Vídeo (genérico)', (SELECT id FROM procedure_categories WHERE slug = 'video_generico')),
  ('77920010', 'Vídeo-endoscopia naso-sinusal com ótica flex. (PACOTE)', (SELECT id FROM procedure_categories WHERE slug = 'video_naso')),
  ('20020040', 'Consulta otoneurologia em consultório', (SELECT id FROM procedure_categories WHERE slug = 'otoneurologia')),
  ('40201309', 'Avaliação endoscópica da deglutição (FEES)', (SELECT id FROM procedure_categories WHERE slug = 'fees'));

-- Insert default doctor and clinic for MVP
INSERT INTO doctors (id, full_name, email, specialty)
VALUES ('00000000-0000-0000-0000-000000000001', 'LUANA CAROLINA FONTANA', 'luanaf18@hotmail.com', 'Otorrinolaringologia');

INSERT INTO clinics (id, name, slug, igut_subdomain)
VALUES
...
INSERT INTO doctor_clinics (doctor_id, clinic_id, active)
VALUES
...
-- Default tax configs
INSERT INTO tax_configs (doctor_id, procedure_category, tax_rate, description, effective_from)
VALUES
...
INSERT INTO monthly_deductions (doctor_id, clinic_id, deduction_type, amount, effective_from)
VALUES
